...
function buildNavGrid(){
  ...
  // Only one resolution now — the flow field solves the whole level from the player in a
  // single pass, so there's no longer any reason to keep a separate coarse grid for
  // long-distance routing (and building it was a meaningful chunk of level load time).
...
//
// Rather than each zombie searching for its own route, the whole problem is solved once from
// the player outward: a Dijkstra pass fills in, for every reachable cell, the true cost of
// walking from that cell to the player. A zombie then just steps to whichever neighbouring
// cell holds a lower value than the one it's standing on.
//
// This replaces both the grid A* and the hand-placed waypoint graph, because it removes the
// causes of the problems those had rather than patching them:
//  - It is correct by construction around concave shapes (C-corridors, courtyards). The field
//    is a complete solve from the target, so "heads toward the player and gets wedged in a
//    dead end" cannot happen — there are no local minima to fall into.
//  - There is no entry-node choice, no path array, no waypoint index and no path expiry, so
//    the oscillation / backtracking / "waiting its turn" failures have nothing to arise from.
//  - Cost is one shared pass rather than one search per zombie per replan, so it gets
//    relatively cheaper the more zombies are alive, instead of more expensive.
// =================================================================
let flowField = null;            // Float32Array aligned with navGridFine: cost from cell -> player
...
function computeFlowField(){
  const grid = navGridFine;
  if(!grid || grid.gridW<2){ flowFieldValid=false; return; }
  const n = grid.gridW*grid.gridH;
  if(!flowField || flowField.length!==n) flowField = new Float32Array(n);
  flowField.fill(Infinity);

  const raw = gridWorldToCell(grid, camera.position.x, camera.position.z);
  const start = nearestOpenCell(grid, raw.gx, raw.gz);
  if(!start){ flowFieldValid=false; return; }

  const W = grid.gridW, H = grid.gridH;
  const startIdx = gridIndex(grid, start.gx, start.gz);
  flowField[startIdx] = 0;
  heapClear();
  heapPush(startIdx, 0);
  ...
  while(heapCell.length>0){
    ...
    if(cCost > flowField[cIdx]) continue;   // stale heap entry, already improved
    if(cCost > FLOW_FIELD_MAX_COST) break;  // far enough out; the rest is beyond interest
    ...
    for(let k=0;k<8;k++){
      ...
      if(drop > STEP_SMOOTH_MAX){ if(drop > LEDGE_DROP_MAX) continue; }
      else if(-drop > STEP_SMOOTH_MAX) continue;
      const stepLen = ((dx!==0&&dz!==0) ? Math.SQRT2 : 1) * grid.cellSize;
      const cost = cCost + stepLen * (1 + grid.wallPenalty[nIdx]);
      // Push the value as it is actually stored: flowField is a Float32Array, so writing a
      // double rounds it slightly. Pushing the unrounded double would then make the
      // stale-entry check above ("cost greater than what's recorded") reject this node when
      // it is popped, leaving the field only partially filled — which looks exactly like
      // zombies freezing beyond some distance for no visible reason.
      if(cost < flowField[nIdx]){ flowField[nIdx] = cost; heapPush(nIdx, flowField[nIdx]); }
    }
  }
  flowFieldValid = true;
  flowFieldCell = { gx:start.gx, gz:start.gz };
}

// Rebuilt only when the player has actually moved to a different cell, and never more often
// than the interval — so it costs nothing while the player is standing still or strafing.
function updateFlowField(){
  const grid = navGridFine;
  if(!grid) return;
  const now = clock.getElapsedTime();
  if(flowFieldValid && (now - flowFieldTime) < FLOW_FIELD_MIN_INTERVAL) return;
  const c = gridWorldToCell(grid, camera.position.x, camera.position.z);
  if(flowFieldValid && c.gx===flowFieldCell.gx && c.gz===flowFieldCell.gz) return;
  computeFlowField();
  flowFieldTime = now;
}

// The world-space centre of the best next cell to move toward, or null when this position has
// no usable field value (outside the solved range, or somewhere genuinely unreachable).
function flowFieldTarget(worldX, worldZ){
  const grid = navGridFine;
  if(!grid || !flowFieldValid || !flowField) return null;
  const c = gridWorldToCell(grid, worldX, worldZ);
  if(!gridInBounds(grid, c.gx, c.gz)) return null;
  const W = grid.gridW, H = grid.gridH;
  const cIdx = gridIndex(grid, c.gx, c.gz);
  const here = flowField[cIdx];

  if(!isFinite(here)){
    // On a cell the field never reached (a sampling gap, or just outside its range): aim for
    // the best finite neighbour so it can rejoin the field rather than stalling.
    let bestVal=Infinity, bgx=-1, bgz=-1;
    for(let k=0;k<8;k++){
      const ngx=c.gx+NEIGH_DX[k], ngz=c.gz+NEIGH_DZ[k];
      if(ngx<0||ngx>=W||ngz<0||ngz>=H) continue;
      const v = flowField[ngz*W+ngx];
  ...
  for(let k=0;k<8;k++){
    ...
    if(flowField[nIdx] < bestVal){ bestVal = flowField[nIdx]; bgx=ngx; bgz=ngz; }
  }
